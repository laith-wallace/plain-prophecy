      ...
      >
        ...
        <div className="reveal-panel-header">
          <div className="reveal-card-label">
            Card {card.number} of {totalCards}
          </div>
        ...
        <div className="reveal-scrollable">
          <div className="reveal-section">
            <div className="reveal-label">What it is</div>
            <p>{card.reveal.what}</p>
          </div>

          <div className="reveal-section">
            <div className="reveal-label">Why it matters</div>
            <p>{card.reveal.why}</p>
          </div>
...
export interface GospelCardHandle {
  triggerSwipe: (dir: 'left' | 'right') => void
}

const GospelSwipeCard = forwardRef<
  GospelCardHandle,
  {
    card: GospelCard
    isNext: boolean
...
>(function GospelSwipeCard(
  ...
  const applyTransform = useCallback((dx: number, flying?: 'left' | 'right') => {
    ...
    if (flying) {
      ...
      l.textContent      = flying === 'right' ? '✓ Resonates' : '? Not sure'
  ...
  return (
    <div
      ref={cardRef}
      className={`swipe-card gospel-swipe-card${promoting ? ' promoting' : ''}`}
      role="button"
      tabIndex={0}
      aria-label={`Card ${card.number}: ${card.title}. Tap to reveal.`}
    ...
    >
      ...
      <div className="swipe-card-inner">
        <div className="card-number">Card {card.number} of {totalCards}</div>
        <div className="card-symbol">{card.symbol}</div>
        <div className="card-title">{card.title}</div>
...
function GospelCompletionScreen({ onRestart, totalCards }: { onRestart: () => void; totalCards: number }) {
  return (
    <div className="completion-screen">
      <div className="completion-symbol">♥</div>
      <div className="completion-overline">{totalCards} of {totalCards} explored</div>
...
export default function GospelClient() {
  ...
  const handleRevealNext = useCallback(() => {
    ...
    setCurrentIndex(i => {
      if (revealCard && gospelCards[i]?.id === revealCard.id) {
        setCardKey(k => k + 1)
        return i + 1
      }
      return i
    ...
    <div className="prophet-layout">
      <header className="prophet-header">
        ...
        <div className="prophet-progress">
          {currentIndex + 1} / {gospelCards.length}
        </div>
        <Link href="/studies" className="prophet-map-btn">
          All studies
        </Link>
      </header>
      ...
      <div className="prophet-progress-bar-track">
        <div
          ...
          style={{ width: `${(completed.length / gospelCards.length) * 100}%`, background: '#C9A84C' }}
        />
      </div>
      ...
      <GospelRevealPanel
        ...
        isLast={currentIndex >= gospelCards.length}
        totalCards={gospelCards.length}
      />
    </div>
  )
